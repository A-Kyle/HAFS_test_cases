...
#ifdef IDEAL_MOD
  use test_control_mod,    only: set_test_hgrid, set_test_vgrid, set_test_tracers, set_test_phys, alpha
  use test_init_mod,       only: ic_test
  use test_cases_mod,      only: init_double_periodic!, init_latlon
#else
...
#ifdef IDEAL_MOD
                   call set_test_hgrid(Atm(n)%gridstruct, Atm(n)%bd, Atm(n)%npx, Atm(n)%npy, Atm(n)%ng)
                   call set_test_vgrid(Atm(n)%npz, Atm(n)%ptop, hybrid)
                   call set_test_tracers(ncnst, Atm(n)%flagstruct%nwat)
                   call set_test_phys(Atm(n)%flagstruct)
                   call ic_test(Atm(n)%u, Atm(n)%v, Atm(n)%w, Atm(n)%pt, Atm(n)%delp, Atm(n)%delz, Atm(n)%q, &
                                Atm(n)%pk, Atm(n)%peln, Atm(n)%pe, Atm(n)%pkz, Atm(n)%phis, Atm(n)%ps, &
                                Atm(n)%uc, Atm(n)%vc, Atm(n)%ua, Atm(n)%va, Atm(n)%ze0, &
                                Atm(n)%ak, Atm(n)%bk, Atm(n)%gridstruct, Atm(n)%domain)
#else !< .not. IDEAL_MOD
...
#ifdef IDEAL_MOD
                if( is_master() ) write(*,*) 'Warn: double periodic IC with IDEAL_MOD may not be supported'
#endif
