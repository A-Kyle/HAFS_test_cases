...
!   use test_cases_mod,      only: test_case
   use test_control_mod,    only: selected_case, alpha
   use test_init_mod,       only: intern_init_test, init_test
...
!  local version of these variables to allow PGI compiler to compile
   ...
   namelist /fv_core_nml/npx, npy, ntiles, npz, npz_rst, layout, io_layout, ncnst, nwat,  &
                         ...
                         regional_bcs_from_gsi, write_restart_with_bcs, nrows_blend, selected_case
#ifdef MULTI_GASES
...
! Read Main namelist
   ...
   !   ierr = check_nml_error(ios,'test_case_nml')
      if (selected_case .NE. 0) then
        call intern_init_test()
      endif
   ...
   ! Read FVCORE namelist 
      ...
      if(is_master()) write(*,*) ' read fv_core_nml, ios=',ios
      ierr = check_nml_error(ios,'fv_core_nml')
   ...
   !   ierr = check_nml_error(ios,'test_case_nml')
      if (selected_case .NE. 0) call init_test(f_unit)
